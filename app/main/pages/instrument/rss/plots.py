...
def _rss_straylight_plot(articulation,start_date, end_date):
    ...
    sql= "select UTStart,mean_z1,mean_z2,mean_z3,mean_z4,mean_z5,mean_z6 from RssStrayLight join FileData using (FileData_Id) join FitsHeaderRss using (FileData_Id) join FitsHeaderImage using (FileData_Id) where UTStart > '{start_date}' and UTStart <'{end_date}' and CAMANG='{camang}'" .format(start_date=start_date, end_date=end_date,camang=articulation)
